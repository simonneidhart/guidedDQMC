program  main
      ! Main program 
        ...
        dimension alat(3,3),rxyza(3,nat),rxyzb(3,nat), rxyzshift(3,nat),rxyzrot(3,nat)


        open(unit=10,file='posinpA.xyz')
        call readfiles(nat,natp,rxyza,alat,symb)
        if (natp.gt.nat) stop 'natpa'
        close(10)
        open(unit=10,file='posinpB.xyz')
        call readfiles(nat,natp,rxyzb,alat,symb)
        if (natp.gt.nat) stop 'natpb'
        close(10)
   

        call  rmsdmin(nat, rxyza, rxyzb, rxyzshift, rxyzrot, rmsd)

        write(*,*) 'rmsd ', rmsd

        open(unit=10,file='posout.xyz')
        write(10,*) 2*nat,'   atomic'
        write(10,*)  rmsd
        do iat=1,nat
        write(10,*)  symb(iat),(rxyzrot(l,iat),l=1,3)
        write(10,*)  symb(iat),(rxyzshift(l,iat),l=1,3)
        enddo
        close(10)
