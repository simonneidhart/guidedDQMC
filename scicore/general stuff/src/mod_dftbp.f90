module mod_dftbp
  ...
  character(len=100), private :: dftb_path
...
contains
  subroutine dftbp_energy_forces(nat, rxyz, fxyz, etot, alat, deralat, atomnames)
    implicit none
    ...
    call create_input_files(nat, alat, atomnumbernames, atomtypes, ntypes, rxyz)
    call run_dftb()
    call get_results(nat, etot, fxyz, deralat)
    deralat = -deralat
  end subroutine dftbp_energy_forces


  subroutine setup_dftbp(path)
    implicit none
    character(len=*) :: path
    character(len=100) :: command
    integer :: i, j
    is_setup = .TRUE.
    dftb_path = trim(adjustl(path))
    call cleanup_dftbp
  ...
  subroutine create_input_files(nat, alat, atomnumbernames, atomtypes, ntypes, rxyz)
    implicit none
    integer :: nat
    !! Number of atoms
  ...
  subroutine get_results(nat, epot, fxyz, deralat)
    implicit none
    !parameters
