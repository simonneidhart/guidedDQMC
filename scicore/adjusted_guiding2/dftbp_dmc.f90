program dmc
  ...
  real(8), allocatable, dimension(:) :: all_e0, mu_step, sigma2_step
  ...
  allocate(fxyz(3,nat))
  allocate(rxyz(3,nat))
  allocate(mu_step(dim))
  allocate(sigma2_step(dim))
  ...
  do n=1,n_steps
    ...
    do j=1,no_walkers
      ...
      !call propagate_unguided(walkers(j), et, delta_t, thread_num)
      if (n .le. 800) then
        call propagate_unguided(walkers(j), et, delta_t, thread_num)
      else
    ...
    if (n .eq. 1000) then
      delta_t = 1.d0
      print*, "delta_t reduced to ", delta_t
    end if
    if (n .le. 800 .and. n .gt. 700) then
      call calc_trial_wf(no_walkers, walkers, mu_step, sigma2_step)
      sigma2 = sigma2 + sigma2_step
      mu = mu + mu_step
    end if
    if (n .eq. 800) then
      sigma2 = sigma2/100
      mu = mu/100
      print*, "no walkers = ", no_walkers
      print*, "mu = ", mu
      print*, "sigma2 = ", sigma2
      print*, "start guiding..."

      open(unit=15, file="walker_positions_hessian_trial", iostat=ios, action="write")
...
subroutine calc_trial_wf(no_walkers, walkers, mu_step, sigma2_step)
  use dftbp_walker
  use parameters
  implicit real(8) (a-h,o-z)
  type(walker), dimension(no_walkers) :: walkers
  real(8), dimension(dim) :: mu_step, sigma2_step

  mu_step = 0.d0
  do i=1,dim
    do j=1,no_walkers
      mu_step(i) = mu_step(i) + walkers(j)%r(i)
    end do
    mu_step(i) = mu_step(i)/no_walkers
  end do

  sigma2_step = 0.d0
  do i=1,dim
    do j=1,no_walkers
      sigma2_step(i) = sigma2_step(i) + (walkers(j)%r(i)-mu_step(i))**2
    end do
    sigma2_step(i) = sigma2_step(i)/no_walkers
  end do

end subroutine calc_trial_wf
