program dmc
  ...
  do n=1,n_steps
    ...
    if (n .eq. 800) then
      print*, "no_walkers= ", no_walkers
      call calc_trial_wf(no_walkers,walkers)
    end if
...
subroutine calc_trial_wf(no_walkers, walkers)
  use dftbp_walker
  use parameters
  implicit real(8) (a-h,o-z)
  ...
  do i=1,dim
    do j=1,no_walkers
      mu(i) = mu(i) + walkers(j)%r(i)
    end do
    mu(i) = mu(i)/no_walkers
    print*, mu(i)
  end do
  ...
  do i=1,dim
    do j=1,no_walkers
      sigma2(i) = sigma2(i) + (walkers(j)%r(i)-mu(i))**2
    end do
    sigma2(i) = sigma2(i)/no_walkers
    print*, sigma2(i)
  end do
