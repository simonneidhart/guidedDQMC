module parameters
  ...
  real(8), allocatable, dimension(:) :: sigma2, new_masses, mu
...
contains
  ...
  subroutine read_hessian(e0_theory, debug_extended)
    ...
    !contains the hessian matrix calculated by dftb+
   ...
   do i=1,dim
     sigma2(i) = 1.d0/(sqrt(2.d0*eigenvalues_full(i+6)*new_masses(i)))
     !sigma2(i) = 0.d0
   end do
