module parameters
  ...
  real(8), allocatable, dimension(:) :: sigma2, new_masses, masses
...
contains
  ...
  subroutine calculate_guiding_wf(e0_theory, debug_extended)
    ...
    real(8), allocatable, dimension(:) :: eigenvalues_full, eigenvalues_full_m
    real(8), allocatable, dimension(:) :: work
    real(8) :: e0_theory, m_in, sigma2_(3,4), omega(3,4), mass(3,4)
    ...
    dim = fulldim !dimensionality reduced by the rotations and tranlations
   ...
   sigma2_(:,1) = (/0.1, 0.2, 0.3/)
   sigma2_(:,2) = (/0.1, 0.2, 0.3/)
   sigma2_(:,3) = (/0.1, 0.2, 0.3/)
   sigma2_(:,4) = (/0.1, 0.2, 0.3/)
   sigma2 = reshape(sigma2_,shape(sigma2))
   ...
   mass = reshape(masses,shape(mass))

   omega = 1/(sigma2_*mass)

   !calculate the theoretical value for e0 based only on the harmonic approximation
   e0_theory = 0
   do i=1,3
     do j=1,4
       e0_theory = e0_theory + 0.5*omega(i,j)
     end do
   end do
...
!write an n x m matrix to the console
  ...
  subroutine energyandforces(nat, rxyz, fxyz, etot, alat, deralat, atomnames, thread)
    implicit none
    integer :: nat, thread, i, j
    real(8) :: rxyz(3,nat), fxyz(3,nat), etot, alat(3,3), deralat(3,3)
    real(8) :: sigma2(3,4), omega(3,4), mass(3,4)
    character(len=2), dimension(nat) :: atomnames

    mass = reshape(masses,shape(mass))
    sigma2(:,1) = (/0.1, 0.2, 0.3/)
    sigma2(:,2) = (/0.1, 0.2, 0.3/)
    sigma2(:,3) = (/0.1, 0.2, 0.3/)
    sigma2(:,4) = (/0.1, 0.2, 0.3/)
    ...
    do i=1,3
      do j=1,4
        etot = etot + 0.5*mass(i,j)*omega(i,j)**2*rxyz(i,j)**2
        fxyz(i,j) = - mass(i,j)*omega(i,j)**2*rxyz(i,j)
      end do
    end do
