...
contains
  ...
  subroutine propagate(this, et, delta_t, thread_num, n)
    ...
    if (n .gt. 20) then
      p_acc = min(1.d0,exp(-dot_product(this%r-r_prop-drift(r_prop,delta_t)*delta_t,this%r-r_prop-drift(r_prop,delta_t)*delta_t))/&
      exp(-dot_product(r_prop-this%r-drift(this%r,delta_t)*delta_t,r_prop-this%r-drift(this%r,delta_t)*delta_t))*psi(r_prop)**2/psi(this%r)**2)
