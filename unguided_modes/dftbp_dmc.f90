program dmc
  ...
  logical :: debug_extended, restart
  ...
  call do_simulation(no_walkers_start, delta_t, n_steps, damping, debug_extended, restart)

end program dmc

  subroutine do_simulation(no_walkers_start, delta_t, n_steps, damping, debug_extended, restart)
  use OMP_LIB
  use mod_dftbp
  use dftbp_walker
  use parameters
  ...
  logical :: debug_extended, restart, half_walker_flag = .FALSE.
  ...
  if (restart .eq. .TRUE.) then !start or restart
    old_no_walkers = 227
    no_walkers = old_no_walkers
    et = -101.806361378394
    e0 = -101.812249727067
  ...
  do n=1,n_steps
    ...
    if (modulo(n,1000) .eq. 0) then !write positions every 1000 steps
