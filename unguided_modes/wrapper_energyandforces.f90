...
subroutine energyandforces(nat, rxyz, fxyz, etot, alat, deralat, atomnames, thread_num)
!! input rxyz and alat must be in atomic units (bohr)
  ...
  !! array containg the chemical symbols of each atom
  character(len=2) :: atomnames(nat)
  !! thread number for the correct choosing the correct forlder in dftp+
  integer :: thread_num
  ...
  call dftbp_energy_forces(nat, rxyz_ang, fxyz, etot, alat_ang, deralat, atomnames, thread_num)
